  ...
  <div class="expert-insights">
    ...
    <div class="insights-section">
      ...
      <div class="usage-list">
        <div class="usage-item" v-for="agent in agentUsage" :key="agent.id">
          <div class="usage-agent">
            <img :src="`/images/${agent.icon}`" :alt="agent.name" class="agent-icon" />
            <span>{{ agent.name }}</span>
          </div>
          <div class="usage-stats">
            <span class="usage-count">{{ agent.calls }} calls</span>
            <div class="usage-bar">
              <div class="usage-fill" :style="{ width: `${agent.percentage}%` }"></div>
            </div>
          </div>
        </div>
      </div>
...
const agentUsage = ref([
  { id: 1, name: 'Customer Support Bot', icon: 'icon-teamhal.png', calls: 892, percentage: 48 },
  { id: 2, name: 'Technical Assistant', icon: 'icon-teamlighthouse.png', calls: 623, percentage: 34 },
  { id: 3, name: 'Sales Agent', icon: 'icon-wordpress.png', calls: 332, percentage: 18 }
]);

// Mock error data
const commonErrors = ref([
  { id: 1, type: 'Timeout', message: 'API response timeout exceeded', count: 45 },
  { id: 2, type: 'Authentication', message: 'Invalid credentials for external service', count: 23 },
  { id: 3, type: 'Rate Limit', message: 'API rate limit exceeded', count: 12 }
]);
</script>
