<template>
  <div class="expert-versions">
        ...
        <div class="version-header">
          <div class="version-info">
            ...
            <div class="version-meta">
              <span class="version-date">Published {{ currentVersion.date }}</span>
              <span class="version-author">by {{ currentVersion.author }}</span>
            </div>
          </div>
          <div class="version-stats">
            <span class="stat-badge success">Live</span>
            <span class="stat-badge">{{ currentVersion.calls }} calls</span>
          </div>
        </div>
    ...
    <div class="versions-list">
      ...
      <div class="version-card" v-for="version in previousVersions" :key="version.id">
        <div class="version-header">
          <div class="version-info">
            <h4>Version {{ version.number }}</h4>
            <div class="version-meta">
              <span class="version-date">Published {{ version.date }}</span>
              <span class="version-author">by {{ version.author }}</span>
            </div>
          </div>
          <div class="version-actions">
            <button class="secondary" @click="viewChanges(version)">View Changes</button>
            <button class="secondary" @click="rollback(version)">Rollback</button>
          </div>
        </div>
        <div class="version-description">
          {{ version.description }}
...
const currentVersion = ref({
  ...
  description: 'Added caching support and improved error handling for timeout scenarios',
  calls: 847
});
...
const previousVersions = ref([
  {
    ...
    description: 'Fixed issue with billing lookup returning incorrect data for certain account types',
    showChanges: false,
    changes: [
  ...
  },
  {
    ...
    date: 'Nov 28, 2023',
    author: 'Shaun Andrews',
    description: 'Added support for multiple Zendesk instances and improved performance',
    showChanges: false,
    changes: [
  ...
  },
  {
    ...
    date: 'Nov 15, 2023',
    author: 'Andy Peatling',
    description: 'Initial version with basic billing lookup functionality',
    showChanges: false,
    changes: [
...
function rollback(version) {
  if (confirm(`Are you sure you want to rollback to version ${version.number}?`)) {
