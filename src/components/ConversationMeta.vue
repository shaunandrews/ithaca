<template>
  <div class="conversation-meta">
    ...
    <div v-if="tags && tags.length > 0" class="conversation-tags" ref="tagsContainer">
      <a href="#" @click.prevent="toggleTagsPopover">
        {{ tags.length }} tag{{ tags.length === 1 ? '' : 's' }}
      </a>
      <div v-if="showTagsPopover" class="tags-popover">
        <div class="tags-list vstack">
          <div v-for="tag in tags" :key="tag" class="tag-item">{{ tag }}</div>
        </div>
      </div>
    </div>
  </div>
...
function handleClickOutside(event) {
  if (tagsContainer.value && !tagsContainer.value.contains(event.target)) {
