        ...
        <div class="preview-header">
            <div class="section-header">
                ...
                <p class="section-description">
                    See how this agent behaves and chat with it.
                </p>
            </div>
        </div>
        ...
        <div class="preview-content">
            <div class="preview-start" v-if="!started">
                <button @click="startPreview">
                    <Play :size="18" />Start preview
                    <span class="shortcut"
                        ><Command :size="12" /> <CornerDownLeft :size="12"
                    /></span>
                </button>
            </div>
                ...
                <li v-for="(event, idx) in revealedEvents" :key="idx">
                    ...
                    <template v-else-if="event.state === 'done'">
                        ...
                        <ul v-if="event.details">
                            <li
                                v-for="(detail, dIdx) in event.details"
                                :key="dIdx"
                            >
                                {{ detail }}
                            </li>
...
<script setup>
    ...
    function startPreview() {
        if (
            !Array.isArray(props.agent?.previewEvents) ||
            props.agent.previewEvents.length === 0
        )
            return;
        started.value = true;
        currentStep.value = 0;
